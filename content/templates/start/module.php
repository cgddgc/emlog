...
function blog_tag($blogid){
	...
	if (!empty($log_cache_tags[$blogid])){
		foreach ($log_cache_tags[$blogid] as $value){
			$tag .= "<a class=\"tag\" href=\"".Url::tag($value['tagurl'])."\">#".$value['tagname'].'</a>';
		}
		echo $tag;
	}
}
...
function neighbor_log($neighborLog){
	extract($neighborLog);?>
	<?php if($prevLog):?>
	&laquo; <a class="prev" href="<?php echo Url::log($prevLog['gid']) ?>">←<?php echo $prevLog['title'];?></a>
	<?php endif;?>
	<?php if($nextLog && $prevLog):?>
	<?php endif;?>
	<?php if($nextLog):?>
		 <a class="next" href="<?php echo Url::log($nextLog['gid']) ?>"><?php echo $nextLog['title'];?>→</a>
	<?php endif;?>
<?php }?>
<?php
//blog：评论列表
